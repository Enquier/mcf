...
class ArtifactService extends ApiClient {
  constructor(authContext, httpContext) {
    super(authContext, httpContext, '/orgs', 'artifact');
  }

  get(orgID, projID, branchID, options) {
    const url = `${this.url}/projects/${projID}/branches/${branchID}/artifacts`;
  ...
  async postBlob(orgID, projID, data) {
    const url = `${this.url}/projects/${projID}/artifacts/blob`;
