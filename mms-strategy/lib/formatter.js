...
// MMS modules
const utils = M.require('api.mms-strategy.lib.utils');
const namespace = utils.customDataNamespace;
const mmsNamespace = 'mms';
...
function mcfOrg(org) {
  ...
  const knownKeys = ['id', 'name', 'public', 'custom'];
  ...
  Object.keys(org).forEach((k) => {
    if (!knownKeys.includes(k)) {
      formatOrg.custom[mmsNamespace][k] = org[k];
    }
  });
...
async function mcfElements(req, elements) {
  ...
  elements.forEach((elem) => {
    ...
    Object.keys(elem).forEach((field) => {
      ...
      if (field === 'ownerId' && elem[field] !== undefined && elem[field] !== null) {
        ...
        if (!elements.map((e) => e.id).includes(elem.parent)) {
          promises.push(ElementController.find(
            req.user,
            req.params.orgid,
    ...
    // not being returned in apiElement()
 ...
 * @param {string} orgId - The id of the org to insert [Optional].
 *
 * @returns {object} An MMS formatted org.
 */
function apiOrg(reqUser, orgObj, orgId) {
  // Get the public data of the org
  const org = getPublicData(reqUser, orgObj, 'org');
  if (org.custom[mmsNamespace].public !== undefined) {
    org.public = org.custom[mmsNamespace].public;
  } else {
    org.public = false;
  }
  if (orgId) {
  ...
  //   id: org.id,
  //   name: org.name,
  //   public: org.custom[mmsNamespace].public,
  //   custom: org.custom
  // };

  // Handle custom
  // if (typeof org.custom[mmsNamespace] === 'object') {
  //   Object.keys(org.custom[mmsNamespace]).forEach((field) => {
  //     formatOrg[field] = org.custom[mmsNamespace][field];
  //   });
  // }
 ...
 */
function apiProject(reqUser, projObj) {
 ...
 */
function apiRef(reqUser, branchObj) {
 ...
 */
function apiElement(reqUser, elemObj) {
 ...
 */
function apiArtifact(reqUser, artifact) {
